...
本系統現在支援**最近一周新聞分析**功能，可透過 Dashboard 和 LINE Bot 查詢：

### Dashboard 功能
- **本周新聞統計卡片**：顯示本周總新聞數、平均情緒、正面/負面新聞數
- **一周情緒趨勢圖**：折線圖顯示過去7天的平均情緒變化
- **本周最熱門股票 TOP 5**：列出本周新聞數最多的前5檔股票

### LINE Bot 指令
```
一周 (或 本周) - 查詢最近一周新聞分析
[股票名稱] 一周 - 查詢特定股票一周分析（新功能）
```
...
**股票一周分析回應內容（例如：`台積電 一周`）：**
- 📅 時間範圍（自動計算為今天往前推7天，如：02/02 - 02/08）
- 📰 該股票總新聞數
